...
while function != 0:
    ...
    if function == 2:
        ...
        print(lerAlunos("..\..\datasets\_alunos.csv"))
    if function == 3:
        print("Tabela de alunos e respetiva média:\n")
        lista = mediaTPC(lerAlunos("..\..\datasets\_alunos.csv"))
        for aluno in lista:
            nome = aluno["nome"]
            media = aluno["media"]
    ...
    if function == 4:
        ...
        lista = gradeTPC(mediaTPC(lerAlunos("..\..\datasets\_alunos.csv")))
        for aluno in lista:
            nome = aluno["nome"]
            media = aluno["media"]
            grade = aluno["escalao"]
    ...
    if function == 5:
        ...
        while subfunction != 0:
            ...
            if subfunction == 2:
                ...
                pp(distribCurso(lerAlunos("..\..\datasets\_alunos.csv")))
            if subfunction == 3:
                print("Tabela da distribuição dos alunos por escalão:\n")
                pp(distribEscalao(gradeTPC(mediaTPC(lerAlunos("..\..\datasets\_alunos.csv")))))
    ...
    if function == 6:
        ...
        while subfunction != 0:
            if subfunction == 1:
                submenuGraph()
            if subfunction == 2:
                graph(distribCurso(lerAlunos("..\..\datasets\_alunos.csv")))
            if subfunction == 3:
                graph(distribEscalao(gradeTPC(mediaTPC(lerAlunos("..\..\datasets\_alunos.csv")))))
            subfunction = int(input("Que gráfico pretende ver?"))
